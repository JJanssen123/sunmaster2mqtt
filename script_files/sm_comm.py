    ...
    def getDailyValues(self, day):
        ...
        for rs in responses:
            if (rs.type == 154):
                ...
                Debug("Response type does not match request 9A", 1)
...
def Read(socket):
    ...
    if (len(bytes) == 0):
        ...
        Debug("No inverter response", 2)
        return rss # empty
    elif (len(bytes) < 9):
        Debug("Incomplete response", 1)
    else:             
        while(len(bytes) > 0):
            if not(bytes[4] == 154 or bytes[4] == 193 or bytes[4] == 182): 
                Debug("Unknown response type", 1)
                break
            else:
                Debug("Response " + hex(bytes[4])[2:].upper(), 2)
                response = bytearray()
                if (bytes[4] == 182): 
